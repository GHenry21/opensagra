...
- [ ] **Wrapper/tray-app in Go** — da costruire al punto 5. Supervisore processi figli (Job Object) + tray + finestra stato/log (webview) + "avvia all'accensione" + X→tray + Esci con conferma (+ avviso "N casse collegate" se server). MVP possibile a ~2 gg senza finestra custom.
  - **Bridge di stampa: esattamente un processo figlio per ID** in `PRINT_BRIDGE_CASSE`. Il demone `opensagra-print-bridge.php` **non ha un lock single-instance**: due processi sullo stesso `--cassa=` si iscrivono entrambi allo stesso topic Mercure e stampano ognuno il job → **scontrini doppi, silenziosi** (osservato 2026-09-09 con due sessioni di sviluppo che avevano avviato il bridge sulla stessa cassa `henry` → due copie identiche di #152). Il wrapper deve deduplicare per ID; in aggiunta o alternativa, guardia nello script (lock file / mutex per `print/cassa/{id}`, esce se già preso).
...
- [ ] **Bridge di stampa — guardia anti-doppione**: il wrapper avvia **un solo** `opensagra-print-bridge.php` per ID in `PRINT_BRIDGE_CASSE`; lo script prende un lock file / mutex per `print/cassa/{id}` all'avvio ed esce se già attivo. Senza questo, due processi sulla stessa cassa = scontrini doppi silenziosi (osservato 2026-09-09, vedi nota nel blocco wrapper in 3g).
