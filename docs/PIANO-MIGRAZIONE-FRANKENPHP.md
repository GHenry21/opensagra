...
| **Packaging / distribuzione** | **Nessun binario embed.** Lo script d'installazione **copia i file** di opensagra nelle cartelle di destinazione. Il codice sorgente resta aperto e ispezionabile (progetto open source). `vendor/` incluso nel pacchetto di release (niente `composer install` sul target). **Aggiunto 2026-09-11**: `opensagra-installer.exe`, un unico eseguibile (`ps2exe -embedFiles`, `packaging/make-installer.ps1`) che al doppio click si autoestrae in `%TEMP%` e lancia lo stesso `install.ps1` di sempre — comodità di distribuzione (un file, un prompt UAC), non un cambio di modello: internamente resta copia file, non un binario dell'app. Controparte: `opensagra-uninstaller.exe`. |
...
| **Realtime (Mercure/SSE)** | **Attivo (2026-09-08, Fase 4).** Hub nel binario FrankenPHP, topic `products`, `EventSource` in `billing.php` con fallback al polling. Cross-macchina in modalità rete via `bin/opensagra-realtime-relay.php` (verificato con la VM). Wiring del punto 5 **fatto** (2026-09-09), relay e bridge come processi figli del wrapper Go: `conf_rete` sincronizza il segreto via DB (`7d064bc`), `install.ps1` genera il blocco `mercure{}` + `MERCURE_JWT_SECRET` + riga `app_config` (`94c4b2f`). **Wrapper e installer, aggiornamento 2026-09-10/12 — molto più avanti di quanto risultasse qui, dettaglio completo in `docs/PIANO-MODIFICHE-WRAPPER.md`**: finestra di stato (HTTP loopback + browser app-mode) con conteggio "N casse collegate" (unione `SHOW PROCESSLIST` + connessioni TCP `ESTABLISHED`), stato MariaDB read-only + tasto "Gestione DB" (**AdminNeo su `/db`, localhost-only, sostituisce phpMyAdmin**), icona/versione nell'`.exe`, conferma "Esci" con TaskDialog, istanza-singola tollerante, autostart, packaging **a-un-file** (`opensagra-installer.exe`, `packaging/make-installer.ps1`) + **disinstaller** (`uninstall.ps1`/`opensagra-uninstaller.exe`). **Testato per davvero su una VM vergine (2026-09-12)**: trovati e corretti 5 bug reali solo in quel giro (MessageBox bloccanti al posto di `Write-Host`/output di processi esterni sotto `ps2exe -noConsole`, quoting `--custom` di winget, `ExecutionPolicy Restricted` di default, fonti winget mai inizializzate). Firma dell'`.exe` (SmartScreen): **decisa NO** (costo non giustificato per ora). **✅ Giro end-to-end del wrapper come client tramite l'installer vero — confermato dall'utente (2026-09-14)**: provato per davvero (non più solo con config scritta a mano). **Il disinstaller aveva dato errori nel test reale — quattro cause reali isolate e corrette lo stesso giorno, riverificato pulito su più cicli install-uninstall-reinstall su VM (vedi Fase 5d).** **Resta anche**: il tentativo di anticipare il dialogo nativo Windows "vuoi fidarti di questo certificato?" (`Register-LocalCaTrust`, commit `eb3d7f2`) non è ancora verificato in VM; pannello nativo macOS/Linux per un eventuale server desktop non-Windows — **deliberatamente rimandato**, non è nei piani reali (server = Windows, Pi = client headless senza interazione). **Roadmap collegata punto 4 — "Fallback locale una-via + push a chiusura cassa" fatto e testato 2026-09-09** (`api/enter_local_fallback.php`, `api/push_local_sales.php`, `bin/opensagra-snapshot.php`, migrazione 004; test DB-level + browser + **end-to-end cross-macchina VM** verdi). **Strumenti visivi persistenti per il push aggiunti 2026-09-10** (`api/sync_status.php`, badge in sidebar, card in `conf_rete.php`, guardia anti-orfani su `set_network_config.php`, auto-heal nel push). **Corretto 2026-09-12** (dopo un brainstorming, senza codice finché il disegno non era chiuso): le vendite pendenti non possono più diventare orfane cambiando modalità (nuovo marcatore `FALLBACK_SESSION_ACTIVE`), e un nodo che passa da indipendente a client non perde più il proprio catalogo (`stock`/`casse_stampanti`/`receipt_config`) — torna a posto da solo, in automatico, allo switch pulito verso Indipendente (`config/catalog_backup.php`, niente pagina dedicata). Scoperto nello stesso ragionamento un problema pre-esistente slegato: nomi prodotto duplicati potevano far scalare lo stock sbagliato in un push (`config/product_name.php`: nome sempre MAIUSCOLO + controllo duplicati). Dettagli nei riquadri "✅ Implementazione (2026-09-09)", "✅ Strumenti visivi per il push (2026-09-10)" e "✅ Vendite mai orfane + catalogo che non si sporca (2026-09-12)" della sezione "Design concreto — Fallback locale una-via". |
...
**Superato in Fase 3 (2026-09-10)**: `install.ps1` non instrada affatto phpMyAdmin (non fa parte del pacchetto spedito) — al suo posto, `/db` serve **AdminNeo** vendorizzato (`tools/adminer.php`), single-file, solo da `127.0.0.1`. Vedi 3g e `docs/PIANO-MODIFICHE-WRAPPER.md` §4 per la scelta e i dettagli.
...
Script usati (erano in `e2e/*.manual.js`, non wired a `npx playwright test` — verifiche una tantum, non una suite CI). **I `fase2d_*` e `fase_final_*` sono stati rimossi il 2026-09-09 con QZ Tray** (referenziavano `api/sign-message.php`, `test_print_bridge.php`, `case 'bridge_qz'` — non più eseguibili); il resoconto qui sotto resta come registro storico, il codice è nella cronologia git.
...
- [x] Stampa scontrino **ESC/POS USB diretta** (tipo `USB`, ex `WIN_USB`/`LINUX_USB`, lato server → `escpos-php` → `WindowsPrintConnector`) — **confermata funzionante** dall'utente su stampante reale. Corroborato anche indirettamente dal confronto `WIN_USB` vs `BRIDGE_NATIVE` in Fase 4 (stesso connettore, scontrini identici in prodotto e impaginazione, vedi "Roadmap collegata" punto 2).
...
- [x] `Caddyfile` generato (`New-CaddyConfig`) — HTTP+HTTPS sempre in parallelo, `tls internal`. **Bug reale trovato in fase di doc-review (2026-09-07)**: il blocco HTTPS elencava solo `https://localhost` — un test precedente che sembrava confermare "funziona anche sull'IP di rete" era in realtà contro il `Caddyfile` di sviluppo (che elenca gia' un IP a mano), non contro quello davvero generato dall'installer. Corretto: `New-CaddyConfig` ora rileva l'IP LAN (stesso meccanismo di `detectLocalLanIp()`, Insidia #8) e lo aggiunge come hostname HTTPS aggiuntivo. **Limite noto, non risolto**: se l'IP cambia dopo l'installazione (rinnovo DHCP), il `Caddyfile` resta con quello vecchio finché non si reinstalla o rigenera a mano — richiede un IP statico/riserva DHCP sul server per essere davvero stabile.
...
>
> **Comportamento della finestra di stato — corretto 2026-09-09 (era sbagliato sotto, vedi nota):**
> - **X chiude/distrugge la finestra di stato normalmente.** Non deve nascondersi: chiuderla è un'azione innocua, perché non è mai lei a possedere i processi supervisionati.
> - **I processi supervisionati (`frankenphp`/`relay`/`bridge:<cassa>`) sono figli del processo Go del wrapper, non della finestra di stato** — restano vivi indipendentemente da quando/se quella finestra è aperta o chiusa. L'unico requisito reale era sempre stato questo ("X chiude la finestra ma non killa i processi"), non che la finestra dovesse restare viva nascosta.
> - **Uscita reale del wrapper = tasto destro sull'icona in tray → Esci → conferma "sei sicuro?"**. Azione deliberata, unico punto che ferma davvero i processi.
...
> - **Linux/macOS:** stesso principio — chiudere la finestra di stato non tocca i processi figli; l'uscita vera resta dal menu tray/barra dei menù con conferma.
>
> ⚠️ **Nota storica (superata):** la prima versione di questa sezione richiedeva **"X = vai in tray, non chiude"** (nascondere invece di distruggere, stile qBittorrent) — un requisito più forte di quanto servisse davvero, che richiedeva intercettare `WM_CLOSE` e annullarne l'effetto di default. `webview_go` non espone quell'hook, quindi implementarlo avrebbe richiesto subclassare la window proc a mano (syscall Win32 fragili, non testabili senza schermo) — è quello che ha bloccato la sessione del 2026-09-08 ("Per wrapper in Go"). Chiarito con l'utente che l'obiettivo vero era solo "non killare i processi in background", già garantito gratis dall'architettura (vedi sopra) — il requisito "nascondi non chiudere" non era mai stato necessario.
>
> **Tecnologia del wrapper (deciso 2026-09-08, finestra di stato rivista 2026-09-09): binario Go**, non PowerShell/WPF. Motivo: a parità di sforzo (~3–4 gg per la versione Windows) il risultato è più robusto proprio dove serve — kill pulito dell'albero di processi figli (Job Object Windows: niente `frankenphp.exe` orfani), restart-su-crash, istanza singola, `.exe` firmabile senza lo stigma "script avvolto in exe" di `ps2exe` verso SmartScreen, e cross-OS reale (stesso codice → tray Windows / barra menù macOS / StatusNotifierItem Linux). Stack: `fyne.io/systray` per la tray, `os/exec` + Job Object per i figli, `go-sql-driver/mysql` per il check `SHOW PROCESSLIST` all'uscita. **L'installer resta WPF/PowerShell/`ps2exe`** (gira una volta, quasi fatto in Fase 3 — non si riscrive): due strumenti, ciascuno adatto al suo ciclo di vita.
>
> **Finestra di stato/log — ridisegnata 2026-09-09: pagina Vue/HTML servita via HTTP dal wrapper stesso, aperta in un browser Chromium in modalità app (`--app=http://127.0.0.1:<porta>`), non `webview` incorporato.** Motivo: `systray` + `webview_go` si contendono il thread principale/message loop di Windows, e `webview_go` non espone un hook su `WM_CLOSE` — nessun modo pulito per farlo coesistere con la tray senza subclassing Win32 fragile. La modalità app di Edge/Chrome (bundlato di serie su Windows 11) dà lo stesso aspetto "finestra piccola senza tab/barra indirizzi" **senza cgo, senza libreria webview, senza conflitto di loop**: è solo un `exec.Command` in più, uguale a come si avvia `frankenphp`. Endpoint serviti dal wrapper (`net/http`, pagina Vue incorporata via `go:embed`): `GET /api/status` (stato/PID/uptime dei figli), `POST /api/restart/<nome>`, `GET /api/logs/<nome>`. Bind solo su `127.0.0.1`. Va gestita lato Go l'istanza singola della finestra app-mode (se già in esecuzione, portarla in primo piano invece di aprirne una seconda).
>
> **Design della pagina — finalizzato 2026-09-09, con render verificati (screenshot HTML, non solo descrizione):** divulgazione progressiva, per non esporre l'utente medio a concetti (frankenphp/relay/bridge) che non conosce, ma senza toglierli del tutto a chi deve fare diagnosi:
> - **Vista di default** (nessun dettaglio tecnico visibile): badge grande "Server: ATTIVO/FERMO" con uptime; bottone dorato **"↗ Apri OpenSagra"** (apre il browser vero su `https://localhost/opensagra`, l'azione usata il 99% delle volte); un solo bottone **"⏻ Ferma/Avvia server"** che agisce su **tutti i processi insieme** (non sui singoli); riga "Questa macchina: SERVER/CLIENT · N casse collegate"; spunta "Avvia all'accensione".
> - **`<details>` "Dettagli avanzati"** (collassato di default, un click per aprirlo): banner di avviso giallo ("fermare un singolo servizio può interrompere stampa o sincronizzazione delle altre casse"), poi una riga per `frankenphp`/`relay`/`bridge:<cassa>` con pallino di stato + PID/uptime + bottoni icona **⏹ Ferma / ▶ Avvia / ⟳ Riavvia** per singolo processo. Il doppio livello (click esplicito + banner) rende il controllo granulare disponibile per il supporto tecnico senza metterlo a portata di click accidentale per l'utente medio.
> - **Implicazione tecnica per il Ferma/Avvia aggregato**: oggi `Supervisor` conosce solo due stati — "gira con auto-restart" o "il wrapper è uscito del tutto" (`cancel()` in `main.go`). Serve aggiungere uno stato intermedio **"in pausa"**: ferma i figli **senza** far scattare il backoff/riavvio automatico e **senza** chiudere wrapper/tray (una flag per-figlio o globale controllata dal loop prima di riavviare). **✅ Fatto** (implementato col resto della finestra di stato, vedi il bullet aggiornato in 3g sotto — è anche lo stato usato per mettere in pausa/riprendere lo snapshot al cambio di ruolo server/client, commit `7efde80`).
>
> **Quando:** era schedulato al punto 5 insieme alla rifinitura di `install.ps1` — **fatto, 2026-09-09/12** (dettaglio giorno per giorno in `docs/PIANO-MODIFICHE-WRAPPER.md`).
>
...
- [x] **Wrapper/tray-app in Go — molto più avanti di come risultava qui; aggiornato dopo revisione del codice e di `docs/PIANO-MODIFICHE-WRAPPER.md` (che traccia in dettaglio i giorni 2026-09-09/12, non solo lo scaffold iniziale).** Compilato con successo 2026-09-09 (Go 1.27 + WinLibs mingw; `go vet` + `go build -ldflags "-H=windowsgui"` puliti al primo colpo), poi esteso ogni giorno fino al 2026-09-12. **Tutto fatto e verificato:** supervisore figli con Job Object `KILL_ON_JOB_CLOSE` + policy di riavvio per exit-code; istanza singola tollerante (lock file con PID + riapertura della finestra esistente se già viva, non solo mutex nudo — `wrapper/lock.go`); **finestra di stato reale** (HTTP loopback + browser app-mode, non più "da fare" — vista semplice + `<details>` "Dettagli avanzati" per-processo, esattamente come progettato); conteggio **"N casse collegate"** corretto per unire `SHOW PROCESSLIST` e connessioni TCP `ESTABLISHED` (il campionamento DB da solo perdeva le casse ferme su `billing.php` col realtime attivo — bug reale trovato testando, `wrapper/cluster.go`); riga di stato **MariaDB** (ping read-only, deciso esplicitamente **di non controllarlo** dal wrapper — niente privilegi elevati, è lo stato condiviso, si autoripara da solo essendo `Automatic`) + tasto **"Gestione DB"** che apre **AdminNeo su `/db`** (`tools/adminer.php`, vendorizzato, instradato solo da `127.0.0.1` nel Caddyfile — sostituisce il link morto a phpMyAdmin, mai instradato dall'installer); versioni FrankenPHP/PHP/MariaDB lette a runtime invece che scritte a mano nel codice; conferma "Esci" con TaskDialog (DPI-aware) + avviso rinforzato se ci sono casse collegate; icona `.ico` multi-size + info versione nell'`.exe`; `--kind=back` anche dopo "Riavvia tutto"; rotazione log dei figli; **build via script** (`wrapper/build.ps1`) + workflow CI dedicato (inerte finché il repo non ha un remote GitHub). **Firma dell'`.exe` (SmartScreen): decisa esplicitamente NO** (costo non giustificato per il numero di installazioni previste; rivalutare con Azure Trusted Signing se la distribuzione si allarga). **Packaging: installer a-un-file `opensagra-installer.exe`** (`packaging/make-installer.ps1`: `git archive` + `vendor/` + wrapper compilato + `ps2exe -embedFiles -requireAdmin`, un solo prompt UAC) **e disinstaller** `opensagra-uninstaller.exe` (ferma il wrapper, disinstalla MariaDB/FrankenPHP/CA locale, esporta un dump DB su Desktop prima di toccare nulla, chiede conferma esplicita salvo `-Force` per smontaggi automatici di VM). **Testato per davvero su una VM vergine (2026-09-12)**, non solo in teoria: 5 bug reali trovati e corretti in quel giro — `Write-Host`/output di `frankenphp.exe`/`mariadbd.exe` non catturato che apriva MessageBox bloccanti sotto l'host minimale `ps2exe -noConsole` (uno per riga, bloccava tutto), quoting `--custom` di winget spezzato dall'array di `Start-Process`, `ExecutionPolicy Restricted` di default che rifiutava il bootstrap, fonti winget mai inizializzate su una macchina vergine (`0x8A150014`). **✅ Test end-to-end del wrapper come client, tramite l'installer vero (non più config scritta a mano) — confermato dall'utente (2026-09-14): provato.** **Il disinstaller aveva dato errori nel test reale — quattro cause reali isolate e corrette lo stesso giorno (Test-Path/Join-Path con path vuoto nell'exe compilato, msiexec al posto di winget per MariaDB, mariadb-install-db al posto di mariadbd --install, Invoke-NativeCaptured per evitare i blocchi da conhost orfano su ps2exe -noConsole), riverificato pulito su più cicli install-uninstall-reinstall su VM — dettaglio completo in Fase 5d.** **Resta aperto**: il tentativo di anticipare il dialogo nativo Windows "vuoi fidarti di questo certificato?" (`Register-LocalCaTrust` → `frankenphp trust`, commit `eb3d7f2`) è scritto ma non verificabile in locale, da confermare in un altro giro di VM. Dettaglio completo in `wrapper/README.md` e `docs/PIANO-MODIFICHE-WRAPPER.md`.
  - **Bridge di stampa — ridisegnato punto-punto (2026-09-12, `fe78d51`)**: la cassa pubblica ora **direttamente sull'hub Mercure del PC-ponte** (`bridge_host`+`bridge_jwt_secret` letti una tantum dalla discovery), non più su quello derivato da `DB_POS_HOST` — prima, un fallback locale (o il centrale mai più tornato su) rendeva il bridge sordo per sempre perché la cassa si spostava sul proprio hub locale, irraggiungibile dal ponte. Topic Mercure ora fisso (`print/bridge`, non più un id per cassa: la stampante la sceglie il payload). Wrapper: `PRINT_BRIDGE_CASSE` è un semplice flag (un solo processo bridge per PC). **Verificato su hardware reale** (Raspberry Pi + stampante USB), sia in sink-file sia con stampa vera, su entrambi i percorsi (stesso PC e `bridge_host` esplicito). Guardia anti-doppione lato script invariata (commit `f3d832d`, `flock(LOCK_EX|LOCK_NB)`).
- [x] macOS/Linux (pannello wrapper): il wrapper Go è cross-OS per costruzione (`status_http.go`+`status_page.html` sono già Go puro); solo tray/dialogo Esci/Job Object/autostart sono Windows-only. **Chiuso come decisione 2026-09-10** (`docs/PIANO-MODIFICHE-WRAPPER.md` §5): nessun server desktop macOS/Linux è nei piani reali (server = Windows col wrapper, Pi = client headless via systemd, nessuna interazione operatore) — si scrive solo se serve davvero, percorso già tracciato (~1 gg Linux, simile macOS). Il test del *resto* dell'app (non del pannello) su Linux/macOS come client/server è tracciato ora in **Fase 5e**.
- [x] ~~Soluzione B (account a bassi privilegi invece di LocalSystem)~~ — **chiuso, non serve più**: col wrapper FrankenPHP gira come utente loggato, il problema LocalSystem non si pone. Resta rilevante solo per il servizio MariaDB (che però non scrive nella webroot).
...
- [x] Produrre `root-CA.crt` + procedura documentata per installarlo sui **tablet** — non uno step di `install.ps1` (è per-dispositivo lato client): **spostato e tracciato in Fase 5c** (guida utente), non resta più come voce isolata qui.
...
- [x] ~~Estendere ad `orders`~~ — **scartato** (2026-09-08): niente vista ordini condivisa per sagre semplici, vedi 3i.
...
- [x] `install.ps1` + `conf_rete` + wrapper: generare `mercure{}` (con `cookie_name`+`cors_origins`) e `MERCURE_JWT_SECRET` sempre; `conf_rete` sincronizza il segreto via DB al passaggio a client; il **wrapper/tray-app** avvia il relay come processo figlio quando `DB_POS_HOST` è remoto (non più servizio WinSW — vedi revisione 3g). Fatto 2026-09-09: `conf_rete` → `7d064bc`; `install.ps1` (`New-CaddyConfig` emette `mercure{}` con `heartbeat 20s` in entrambi i siti, `New-EnvFile` genera `MERCURE_JWT_SECRET` 256-bit hex + righe `_REMOTE`/`PRINT_BRIDGE_CASSE`, `Get-OrNewMercureSecret` lo riusa al rilancio, `Set-MercureSecretInDb` scrive la riga `app_config` dopo il provisioning) → `94c4b2f`; relay come figlio del wrapper → `d4d758e` (`children.go`). **Testato su VM vergine reale il 2026-09-12** (vedi 3g) e con il wrapper come client via installer il 2026-09-14 — non più solo "codice completo".
- [x] **Bridge di stampa — guardia anti-doppione**: il wrapper avvia **un solo** `opensagra-print-bridge.php` per ID in `PRINT_BRIDGE_CASSE` (`children.go`); lo script prende un lock per `print/cassa/{id}` all'avvio ed esce se già attivo. **Fatto 2026-09-09 (`f3d832d`):** `bridgeAcquireLock()` → `flock(LOCK_EX\|LOCK_NB)` su `<tmp>/opensagra-print-bridge-<cassa>.lock`, `exit(0)` se già preso (uscita pulita, il supervisore non la tratta come crash).
- [x] **Scalino 0 + Scalino 1** (recovery DB-down, indipendenti) — **FATTI 2026-09-09**. Client `pages/billing.php`: `checkout()` async con retry backoff 1→5 s per ~20 s sullo **stesso** payload, poi toast persistente *"Server non raggiungibile — vendita NON registrata. Riprova."* (carrello intatto); `idempotency_key` (`crypto.randomUUID` + fallback v4) nel payload, `checkoutBusy` blocca il doppio invio. Server `print/print_receipt.php`: `ensureVenditeIdempotencyColumn()` (colonna `vendite.idempotency_key VARCHAR(36) NULL` + indice `uniq_vendite_idempotency_key` UNIQUE) + `tryIdempotentReplay()` (chiave già vista → ristampa dallo storico, nessuna nuova vendita né decremento scorte), chiamato prima della transazione **e** nel `catch` dopo `rollback()` per la corsa di due invii simultanei. Migrazione `config/migrations/003_vendite_idempotency_key.php` + colonna/indice in `config/pos.sql`. Verificato: curl (2 POST stessa chiave → 1 riga, 2ª risposta `idempotent_replay`) + `e2e/checkout-resilienza.spec.js` (3 scenari, verde chromium/firefox/webkit, POST intercettate → zero scritture sul DB). Dettaglio in "Sotto-punti di sicurezza".
- [x] **Fallback locale una-via + push a chiusura cassa** (roadmap collegata punto 4, **dopo relay e bridge di stampa nativo**): A) trigger + swap una-via su `127.0.0.1`; B) snapshot locale nel processo sottoscrittore — base periodica ~2-5 min + refresh anticipato su evento `products`, mutuamente esclusivo con lo stato di fallback; C) push a `chiusura cassa` (UUIDv7/id-per-nodo + `idempotency_key`, FK rimappate) con bottone "N vendite da sincronizzare" persistente se il centrale è ancora giù; D) replay decrementi stock; E) test 4 scenari. Design completo in "Design concreto — Fallback locale una-via + push a chiusura cassa" (+ sottosezione "Punto B — meccanismo dello snapshot"). **Fatto e testato 2026-09-09, corretto 2026-09-12 (vendite mai orfane + catalogo che non si sporca), runbook end-to-end ripetuto su VM e confermato dall'utente il 2026-09-14 — il meccanismo regge, restano solo warning UI da rifinire** (vedi nota sotto).
...
- ~~`install.ps1` non genera il blocco `mercure`/il segreto JWT; `conf_rete` non sincronizza `MERCURE_JWT_SECRET` via DB~~ → **FATTO 2026-09-09** (`conf_rete` `7d064bc`, `install.ps1` `94c4b2f`). Il relay non è più un servizio WinSW ma un processo figlio del wrapper Go (scaffold `d4d758e`). Non testato su VM pulita.
...
1. **Relay realtime** — ✅ **FATTO e verificato 2026-09-08.** `bin/opensagra-realtime-relay.php`: processo CLI, si iscrive in streaming all'hub del **server** (header `Authorization: Bearer`, niente cookie/CORS: non è un browser), parsa l'SSE riga per riga e **ri-pubblica ogni evento sull'hub locale**; `billing.php` resta identico (parla sempre col suo hub locale). Riconnessione con `Last-Event-ID` + backoff; `Low-Speed-Time` 45s per accorgersi di uno stream morto; esce subito (idle) se `DB_POS_HOST` è locale; log su STDERR (WinSW lo cattura).
   ...
   - **Segreto condiviso ✅ FATTO 2026-09-09 (Opzione A, commit `7d064bc`):** niente più copia a mano. `config/app_config.php` (nuovo, tabellina k/v `app_config`); `config/mercure.php::mercureSecretForHub($hubUrl)` sceglie il segreto: hub `localhost`/`127.0.0.1` → `MERCURE_JWT_SECRET` locale, altro host → nuovo `MERCURE_JWT_SECRET_REMOTE` (fallback al locale). `mintMercureJwt(...,$secret='')` param aggiunto; `publishMercureUpdate` firma col segreto dell'hub target. `publishProductsChanged` → `seedServerMercureSecret()` (static-once, solo se host locale) pubblica il segreto del server in `app_config`. `api/set_network_config.php`: mode client → legge `MERCURE_JWT_SECRET` dal DB del server e lo salva come `MERCURE_JWT_SECRET_REMOTE` locale (risposta con `mercure_secret_synced`); mode indipendente → azzera il `_REMOTE` e semina il proprio segreto nel DB. Relay e print-bridge: guardia + `mint_jwt` usano `mercureSecretForHub(mercureHubUrl())`. **Il Caddyfile del client non si tocca** — l'hub locale del client resta sul suo `MERCURE_JWT_SECRET`. Caveat chicken-egg: un server nuovo semina la riga alla prima modifica prodotti (o al primo `conf_rete → indipendente`); **`install.ps1` ora la scrive direttamente** (`Set-MercureSecretInDb`, commit `94c4b2f`). Non ancora testato cross-macchina VM. Avvio del relay: **processo figlio del wrapper Go**, non più un servizio — scaffold `d4d758e` (`children.go`), da compilare.
...
2. **Bridge di stampa nativo** — ⏳ **plumbing FATTO e verificato (2026-09-08), manca test con stampante reale.**
   ...
   - **Fusione tipi `WIN_USB` + `LINUX_USB` → `USB` (2026-09-08)** ✅ — col modello wrapper l'host che stampa è già Windows *o* Linux (build per-OS), quindi la scelta OS non è un dato per-cassa: era solo un modo per sbagliare (scegliere `LINUX USB` su Windows → discovery a vuoto). Ora nel select "Tipo Stampante" c'è **una sola** voce `USB / STAMPANTE LOCALE`. La risoluzione Windows-vs-Linux avviene a runtime nell'unico collo di bottiglia `getPrinterConnector()` (`config/get_printer.php`, nuova `usbConnectorForTarget()` su `PHP_OS_FAMILY`) e in `print/test_print_usb.php`. Config **portabile**: la stessa riga stampa da un PC Windows o da un Pi Linux senza riconfigurare. `WIN_USB`/`LINUX_USB` restano `case` legacy che forzano il connettore storico (righe pre-fusione: continuano a funzionare, nessuna migrazione DB); il modal le rimappa a `USB` all'apertura (migrazione pigra: il DB cambia solo se l'utente salva). `bridgeNativeTargets` accetta `USB` + i due alias. `pages/conf_casse.php` espone `PHP_OS_FAMILY` a Vue (`window.__OPENSAGRA_SERVER_OS__`) per scegliere quale discovery lanciare (`caricaStampantiWindows`/`caricaStampantiLinux`) e che hint mostrare. `casse_stampanti.tipo_stampante` è `varchar(50)` → nessun vincolo enum da toccare.
   - **Modello `BRIDGE_NATIVE` "a una riga" + discovery via proxy (2026-09-08, 2ª iterazione)** ✅ — dopo aver valutato e scartato mappa-stampanti su file locale e redesign bridge-centrico, si è scelto il modello **a una riga**: la cassa `BRIDGE_NATIVE` porta *direttamente* la stampante del ponte (non punta a una seconda cassa). Nuove colonne runtime `casse_stampanti.bridge_printer_type` (USB/RETE/…) e `bridge_topic` (id-topic Mercure, default `cassa_id`); `nome_indirizzo` = nome/IP stampante, `bridge_host` (ex `qz_host`, rinominata il 2026-09-09) = IP del ponte, usato **solo dalla discovery**, non in stampa. `config/printer_connectors.php` (nuovo, zero DB): `getPrinterConnectorFromSpec()` + `bridgeNativeRouting()`. `print_receipt.php`/`print_stat_receipt.php`/`test_print_bridge_native.php` pubblicano `printer`/`printer_type`/`printer_port` nel payload; `bin/opensagra-print-bridge.php` costruisce il connettore **dal payload** — nessun accesso al DB (verificato: parte e stampa con MariaDB irraggiungibile). Fallback "a due righe" invariato (payload senza `printer` → `bin` risolve dalla riga della cassa-ponte, connessione DB non fatale). **Discovery remota ripristinata** (era `remotePrinterList()`, mai committata): action `remote_list_printers` in `api/stampanti.php` (`config/printer_discovery.php`), fetch server→server verso `http(s)://<ip>/api/stampanti.php?action=list_*_printers`, guardia anti-SSRF (solo IPv4 LAN / `*.local`), 2s connect-timeout, https→http. `list_*_printers` spostate **prima** del `require get_db_connection.php` → girano su un ponte non ancora in client mode. `getWindowsPrinters()` ora **filtra le stampanti virtuali** (PDF/XPS/OneNote/Fax via `DriverName`/`PortName`/nome) — migliora anche la discovery locale. `conf_casse.php`: ramo `BRIDGE_NATIVE` in stile QZ (IP del ponte + "Ricerca Stampanti" + toast con bottone "Usa questa" per stampante, riusa `toast.js` `options.actions`; 1 risultato → auto). Verificato end-to-end su host: create/update/delete via API con le nuove colonne; `test_print_bridge_native` → topic da `bridge_topic` + `printer` nel payload → bridge riceve e scrive ESC/POS valido; proxy `remote_list_printers` contro l'istanza locale OK, guardia SSRF respinge IP pubblici/hostname; **stampa REALE sulla POS-80C USB attraverso il bridge col modello a una riga — carta uscita, confermato dall'utente** (`WindowsPrintConnector` costruito dal payload, nessun DB); `e2e/bridge-native-config.spec.js` (nuovo) verde su chromium/firefox/webkit (campi + ID-topic auto, discovery via proxy, toast "Usa questa", switch USB↔RETE, salvataggio con check DB, riapertura in modifica); suite e2e completa 15/15 (incl. `mercure-realtime` non-regressione). Fix collaterale: `.printer-modal` ora scrolla (max-height + `overflow-y` sul body) — il ramo BRIDGE_NATIVE aggiunge campi e il footer "Salva" finiva fuori viewport su schermi bassi.
   - **Cross-macchina VM (2026-09-08)** ✅ — copiati sulla VM (`C:\opensagra`) gli 8 file del modello a una riga; VM in modalità client (`DB_POS_HOST`=host, DB condiviso, `MERCURE_JWT_SECRET` uguale). Cassa `BRIDGE_NATIVE` creata via API host (riga vista da entrambe); bridge `--cassa=vm_bridge_node` avviato **sull'host** (POS-80C); `test_print_bridge_native` invocato **sul PHP della VM** → `publishMercureUpdate` → `mercureHubUrl()` VM ricava l'hub dell'host → evento attraversa la rete → bridge host stampa dal payload (`scontrino stampato (121 byte)`, `WindowsPrintConnector`, nessun DB lato bridge). `list_win_printers` sulla VM (senza stampanti) → `{printers:[],bridge_id:""}` pulito, nessun fatal dal require di `printer_discovery.php`. Cassa di test rimossa.
   - **Rimozione QZ Tray (2026-09-09)** ✅ **FATTO.** Cancellati `assets/js/qz-tray.js` + `assets/js/qz-helper.js` + `api/sign-message.php` + `config/qz_key_path.php` + `config/installa_certificati_qz.php` + `cert/cert.pem`/`cert.cer` + `print/test_print_bridge.php`. Tolti tutti i rami/funzioni QZ da `pages/billing.php` (`printBridgeViaQz`, `ensureQzConnected`, `ensureQzLibraryLoaded`, `normalizeQzHost`, `loadScript`, `normalizeJsonResponse`, `case 'bridge_qz'` in checkout e ristampa), `pages/stat_vendite.php` (stessi metodi + `case bridge_qz`), `pages/conf_casse.php` (campo "Host QZ Tray", bottone "Ricerca Stampanti" QZ, `<select>` stampanti BRIDGE, `fetchQzPrinters`/`refreshBridgePrinters`/`triggerBridgeSelectHighlight`/`onBridgePrinterSelectChange`, data `bridgePrinters`/`bridgeSelectValue`/`highlightBridgeSelect`/`loadingQzPrinters`, rami `BRIDGE` in `getTestEndpointByType`/`testPrinterConfig`/`onTipoStampanteChange`/`openModal`/`saveRecord`/`beforeUnmount`; header colonna "QZ Host" → "IP ponte"). `config/get_printer.php`: rimosso `case 'BRIDGE'`. `print/print_receipt.php` + `print/print_stat_receipt.php`: rimosso il ramo `BRIDGE`/`bridge_qz`. `install.ps1`: rimossa `Install-QZTray` e la sua chiamata (era il 95%). **Dati:** unica riga `tipo_stampante='BRIDGE'` (`henry`, dev) migrata a `BRIDGE_NATIVE` una-riga (`bridge_printer_type=USB`, `bridge_topic=henry`, `nome_indirizzo=POS-80C`). Verifica: PHP lint pulito, le 3 pagine caricano senza errori, suite e2e verde (incl. `bridge-native-config` 3 browser).
   - **Pulizia residui QZ + rename `qz_host` → `bridge_host` (2026-09-09)** ✅ — (1) rimossi 9 script `e2e/fase2d_*`/`fase_final_*.manual.js` + 5 PNG (referenziavano endpoint QZ cancellati, feature superata); (2) tolta la voce difensiva `formatTipoStampante` `'BRIDGE': 'BRIDGE (QZ)'` (nessuna riga `BRIDGE` esiste più) e un commento stantìo in `conf_casse.php`; (3) **colonna `casse_stampanti.qz_host` rinominata `bridge_host`** — il nome ingannava (da Fase 4 è solo l'IP del PC-ponte per la discovery, mai QZ). Nuova migrazione `config/migrations/002_qz_host_to_bridge_host.php` (`CHANGE COLUMN`, dati preservati; aggiunge anche `bridge_printer_type`/`bridge_topic`, ora pure in `config/pos.sql`). `ensure*` in `api/stampanti.php` e `config/get_printer.php` fanno la rinomina idempotente a runtime (`SHOW COLUMNS` + `CHANGE COLUMN`) per i DB che non lanciano la migrazione. Rinominati end-to-end: chiave wire API (`$input['bridge_host']`), SELECT/INSERT/UPDATE, chiave array di `getPrinterSettings`, tutto il JS di `conf_casse.php` (sort key, `<td>`, `modalData`, payload create/update/test, discovery). `AFTER qz_host` nelle vecchie `ADD COLUMN` legacy → `AFTER porta`. Migrazione lanciata sul DB dev (riga `poop` `127.0.0.1` e `henry` `192.168.88.224` conservate); `api/stampanti.php?action=list` → `bridge_host` nel JSON; suite e2e 18/18 (6 skip pre-esistenti).
3. **Avviso "il server sta per fermarsi"** ✅ **FATTO 2026-09-08; trigger del wrapper compilato e verificato (aggiornamento: non più solo scaffold, vedi 3g/`PIANO-MODIFICHE-WRAPPER.md`).** Topic `cluster/announce`, una-via server→client. Il protocollo Mercure non trasporta il topic al subscriber → ogni payload si auto-descrive con un campo `type` (`'products'` | `'announce'`); `publishProductsChanged()` ora lo aggiunge. `config/mercure.php::publishClusterAnnounce($kind,$eta,$msg)` — `$kind`: `shutdown` | `back`. Trigger: CLI `bin/opensagra-announce.php --kind=shutdown [--eta=30] [--message=…]` (a mano) **oppure il wrapper Go** — `announceShutdown()` prima di terminare FrankenPHP e `announceBack()` all'avvio/dopo "Riavvia tutto". Relay: `RELAY_TOPICS` include `cluster/announce`, `on_event` sceglie il topic locale dal `type` (mappa `RELAY_TYPE_TO_TOPIC`), guardia `_relay` invariata. `api/mercure_subscribe_token.php`: scope `subscribe` = `['products','cluster/announce']`, risposta con `topics[]` (retrocompat `topic`). `pages/billing.php`: EventSource con un `topic=` per voce; `onmessage` instrada su `type`; `handleClusterAnnounce()` → `serverAnnounce` → **toast persistente** (aggiornato 2026-09-14, vedi sotto), chiudibile a mano, si pulisce da solo al primo evento non-announce (server tornato) o dopo 10 min. Cross-macchina: dipende dal segreto Mercure condiviso, ora sincronizzato via DB (punto 5).

   **✅ Convertito in toast (2026-09-14)**, su segnalazione dell'utente durante il collaudo: prima era una barra fissa (`.server-announce` in `billing.css`, colore `--mc-danger`), ora `window.showToast(text, 'error', { duration: 0 })` — persistente (nessun auto-dismiss, richiede chiusura manuale o l'arrivo di `kind=back`/di un evento non-announce), stessa `.toast-close-btn` degli altri avvisi dell'app (`toast.js`). `handleClusterAnnounce()`/`clearServerAnnounce()` tengono `_serverAnnounceDismiss` (la funzione restituita da `showToast`) per chiuderlo/sostituirlo; `serverAnnounce` resta come stato interno (usato per decidere se un evento non-announce deve pulire l'avviso) ma non guida più alcun template. CSS `.server-announce*` rimosso da `billing.css`. Test aggiornato: `e2e/cluster-announce.spec.js` verifica `#message .toast.error` invece di `.server-announce`.
4. **Fallback locale una-via + push a chiusura cassa** — ✅ **FATTO E TESTATO 2026-09-09** (implementazione + test DB-level + browser + end-to-end cross-macchina sulla VM). Mitigazione del buco prolungato del DB centrale in modalità rete. Design completo, tabella A-E, spigoli e stima nella sezione **"Design concreto — Fallback locale una-via + push a chiusura cassa"** più sotto; **stato di completamento e scelte concrete** nel riquadro "✅ Implementazione (2026-09-09)" in fondo a quella sezione. Prerequisiti indipendenti **già fatti (2026-09-09)**: **Scalino 0 + Scalino 1** (vedi "Sotto-punti di sicurezza") — il push del punto C riusa lo stesso `idempotency_key` per rendere sicuro un push interrotto e ripreso.
...
- **Mitigazione server-down: dati salvati comunque in locale e ripristinati alla riapparsa del servizio.** Se il server DB cade durante il servizio, il client oggi non può fare checkout (`new mysqli()` verso `DB_POS_HOST` fallisce e `print/print_receipt.php` muore prima di fare qualsiasi cosa — l'operatore resta col cerchio che gira e non sa se la vendita è passata). Approccio a scalini, analizzato 2026-09-08:
  ...
  **Scalino 0 — rendere il fallimento visibile e non distruttivo.** ✅ **FATTO 2026-09-09** (`pages/billing.php`).
  - Checkout fallito per DB/rete irraggiungibile → stato UI esplicito: *"Server non raggiungibile — vendita NON registrata. Riprova."* (toast persistente con la × per chiudere). Prima era un 500 grezzo / spinner appeso.
  - `checkout()` è ora `async`: `_postCheckoutOnce()` classifica la risposta **retriabile** (status `0` / `500` / `502-504` / timeout — il "DB down" tipico di LAN) vs **non retriabile** (`403`/`409` = regola di business: metodo pagamento, carrello vuoto, scorta esaurita — nessun retry, si mostra l'errore). Sui retriabili: loop di retry con backoff `1→2→4→5…` s per ~20 s sullo **stesso** payload (stessa `idempotency_key`), poi il toast esplicito. Il carrello **non** viene toccato finché non c'è successo; `checkoutBusy` (data) + `:disabled` sul `#checkout-btn` bloccano un secondo invio durante tutta la finestra.
  - Nessun cambiamento server-side oltre alla colonna dello Scalino 1.
  - Verifica: `e2e/checkout-resilienza.spec.js` (POST a `print_receipt.php` intercettate da Playwright → nessuna scrittura sul DB condiviso), 3 scenari verdi su chromium/firefox/webkit: (1) 2×503 poi 200 → 3 tentativi stessa chiave, toast successo, carrello svuotato; (2) 503 perpetuo → ≥3 retry stessa chiave, toast "NON registrata", carrello intatto, pulsante disabilitato poi riabilitato; (3) 409 → 1 solo tentativo, carrello intatto.

  **Scalino 1 — chiave di idempotenza sul checkout.** ✅ **FATTO 2026-09-09** (`print/print_receipt.php`).
  - Il client genera un UUID (`crypto.randomUUID`, fallback v4 "a mano") per ogni tentativo di checkout e lo manda in `payload.idempotency_key`; lo stesso UUID viaggia identico a tutti i retry dello Scalino 0.
  - Server: `ensureVenditeIdempotencyColumn()` crea `vendite.idempotency_key VARCHAR(36) NULL` + indice `uniq_vendite_idempotency_key` UNIQUE (NULL multipli ammessi → vendite storiche e senza chiave convivono). `tryIdempotentReplay()`: se la chiave esiste già → ricarica gli item da `dettagli_vendita`, richiama `routingStampa` (ristampa lo scontrino), risponde `{success:true, idempotent_replay:true, id_vendita}` — **nessuna** seconda vendita, **nessun** decremento scorte. Chiamato **prima** della transazione e di nuovo nel `catch` dopo `rollback()`: se due invii simultanei con la stessa chiave arrivano insieme, il secondo `INSERT` perde sull'indice UNIQUE (errore `1062`) e viene servito come replay invece che con un `409` grezzo. L'`INSERT INTO vendite` include ora `idempotency_key` (bind `null` se assente/malformata — regex `^[A-Za-z0-9._-]{8,36}$`).
  - Persistenza schema: migrazione una-tantum `config/migrations/003_vendite_idempotency_key.php` (idempotente) + colonna e indice aggiunti a `config/pos.sql` per le installazioni pulite. La `ensure*` a runtime copre i DB che non lanciano la migrazione.
  - Rende sicuro **ogni** retry — quello automatico dello Scalino 0 e anche il "riprovo a premere" manuale che l'operatore fa già oggi quando non è sicuro che sia passata. Risolve il rischio *doppio invio*. È anche il prerequisito del push del punto 4.C (push interrotto e ripreso). Verifica: curl (2 POST stessa chiave → 1 sola riga in `vendite`, 2ª risposta `idempotent_replay` con stesso `id_vendita` e stesso ESC/POS; POST senza chiave → path legacy invariato).
...
| | Pezzo | Peso |
|---|---|---|
| **A** | **Trigger + swap una-via**: "centrale irraggiungibile da > N s" (in implementazione N = 90 s — oltre un riavvio di servizio / un blip WiFi, ma non 2-3 min di stallo in cassa; lo Scalino 0 copre già i secondi) → riscrive `variabili.env` su `127.0.0.1`, soft reload di `billing.php` (il carrello è già in `localStorage`, sopravvive). **Una sola direzione**: nessun ritorno automatico alla rete durante la serata. | piccolo-medio |
...
#### ✅ Implementazione (2026-09-09)

Fatto tutto A–E. Scelte concrete e scostamenti dal design:

- **Marker di stato:** una sola riga in `config/variabili.env`, `FALLBACK_ORIGIN_HOST`. Vuota = esercizio normale; valorizzata con l'IP del server (il vecchio `DB_POS_HOST`) = "sto girando in locale". La leggono `env_reader.php` (`loadPosEnvVars()['fallback_origin_host']`), lo snapshot (per fermarsi), `print/print_receipt.php` (per marcare le vendite) e il push (per sapere dove ricaricarle). Nessun `node_id` (deciso con l'utente: **solo `idempotency_key`** — ogni vendita in fallback ne ha già uno univoco dallo Scalino 1, l'indice `UNIQUE uniq_vendite_idempotency_key` sul centrale deduplica un push ripreso; l'INSERT sul centrale prende un nuovo AUTO_INCREMENT e i dettagli si rimappano su quello).

- **Punto A — trigger.** `api/enter_local_fallback.php` (POST): guardie 409 se non è in modalità rete, 422 se il DB locale non è pronto (connessione a `127.0.0.1` + `app_config['snapshot_last_ok']` presente + `stock` non vuoto); poi `setEnvValue` `FALLBACK_ORIGIN_HOST` = host attuale, `DB_POS_HOST` = `127.0.0.1`. Idempotente (ramo "already"). `api/set_network_config.php` azzera `FALLBACK_ORIGIN_HOST` su ogni switch manuale. Rilevamento in `pages/billing.php`: `noteServerReachable()`/`noteServerUnreachable()` agganciati a `pollProductsVersion()` (già gira) e alla fine di `checkout()`; `maybeArmLocalFallback()` fa la POST + `window.location.reload()` (il carrello è in `localStorage`, come per lo Scalino 0) quando `Date.now() - _serverDownSince > fallbackAfterMs()` (default **90 s** — oltre un riavvio del servizio MariaDB e i blip WiFi, senza tenere l'operatore fermo troppo a lungo; era 150 s, ridotto il 2026-09-09 — override `localStorage['fallback_after_ms']` per i test). **Produzione = automatico e silenzioso** (solo `console.info`, nessun toast/banner: "l'operatore processa ordini e non vede messaggi strani" — richiesta esplicita). Hook di test: `window.__opensagraForceLocalFallback()` e `window.__opensagraTestSimulateServerDown(ms)`.
...
- **Test.** `e2e/local-fallback.manual.php` +5 scenari (decisione `$autoReturnToNetwork` per
  ...
  `conf_rete.php` caricano dopo il refactor di `env_reader.php`.

- **✅ Runbook end-to-end ripetuto sulla VM (confermato dall'utente, 2026-09-14).** Il giro
  completo col nuovo gate (niente più `force`/409, debito che si azzera solo al push — vedi
  sopra) è stato rifatto sulla VM: **il meccanismo regge**, nessun problema strutturale trovato.
  Restano da sistemare solo alcuni **messaggi di warning** mostrati durante il collaudo
  (rifinitura UI/testo, non logica) — non bloccante, non ancora dettagliato in una voce a parte.
...
Aperta il 2026-09-14. A differenza delle fasi precedenti (infrastruttura/funzionalità), questa è tutta orientata a portare il progetto da "funziona, testato dallo sviluppatore" a "qualcun altro lo può installare/vedere/trovare da solo".

### 5a. Pulizia file e cartelle — primo passo, prima di tutto il resto ✅ COMPLETATA (2026-09-15)

- [x] **Rimossi** (nessun riferimento nel codice live, verificato con grep prima di cancellare): l'intera cartella `archive/legacy/` (vecchia implementazione QZ Tray pre-refactor, 13 file incl. una cattura raw ESC/POS `USB001`), `bt.html` e `navbar example.html` (demo/test standalone alla radice del repo, mai linkati da nulla), `e2e/mdns_responder_test.manual.js` (test di un approccio mDNS che il piano stesso registra come provato e scartato — Appendice C/E — dipendente da un pacchetto npm nemmeno in `package.json`). **Tenuti** invece gli altri `.manual.js`/`.manual.php` in `e2e/`: sono strumenti di verifica tuttora attivi per funzionalità in uso (fallback locale, catalog backup, product-name, conf_rete, download PDF, screenshot della guida), non residui.
- [x] **Segreti trovati ed eliminati dalla cronologia, non solo dal working tree**: `archive/legacy/private-key.pem` e `digital-certificate.key` erano due chiavi private vere, committate nel commit di baseline (`96fa090`) e mai più toccate — il vecchio certificato QZ Tray. Decisione esplicita dell'utente: riscrivere la storia ora (repo ancora senza remote, finestra sicura) invece di lasciarle recuperabili una volta pubblicato su GitHub (Fase 5b). Eseguito con `git-filter-repo --invert-paths --path archive/legacy/private-key.pem --path archive/legacy/digital-certificate.key` (installato via `pip install --user git-filter-repo`, non presente prima). Verificato dopo: i due file non compaiono più in nessuno dei 194 commit (`git log --all` + scan di tutti gli oggetti via `git cat-file --batch-check`), nessun remote configurato (nulla da forzare), working tree pulito. Backup di `.git` fatto in scratchpad prima dell'operazione, per prudenza.
- [x] Rimosso anche `.DS_Store` alla radice (file di sistema macOS, non tracciato ma inutile).
- [x] **Committato** (`655a4e9`): tutte le rimozioni sopra + l'aggiornamento di questa sezione del piano.
- [x] **Secondo giro di verifica, dopo il commit** — niente dato per scontato: `git grep` su tutti i file tracciati per `BEGIN...PRIVATE KEY` (zero risultati), per pattern `password=`/`secret=`/`api_key=` con valori non-placeholder nei `.php`/`.ps1`/`.js` (zero risultati fuori da esempi/placeholder), e per file tipo `*.exe~`/`*.bak`/`*.orig`/`*.tmp` (zero risultati). Confermato anche che `cert/cert.pem` (certificato QZ attivo) e `private/key.pem` (chiave di firma, fuori dalla webroot) **non sono tracciati per design** — generati per-installazione, coerente con l'Appendice C.
- **Nota per un giro successivo, non affrontata qui**: `uploads/` contiene foto reali di prodotti/loghi di eventi passati, committate come dati "seed" — non un residuo di sviluppo in senso stretto, ma da valutare se vada bene renderle pubbliche insieme al codice quando si arriva alla Fase 5b.

### 5b. Pubblicazione su GitHub

- [x] **Solo la guida, anticipata (2026-09-15)**: repo pubblico creato (`github.com/GHenry21/opensagra`, via `gh` CLI installato al volo) e pushato **per rendere consultabile `docs/GUIDA-UTENTE.md`** con le immagini renderizzate — non perché lato codice/Windows sia dichiarato completo. History già ripulita da chiavi private in Fase 5a prima di questo push.
- [x] **Prima release pronta (2026-09-16)**: `.github/workflows/release.yml` — un tag `v*` fa scattare una build su `windows-latest` che compila wrapper+installer+disinstaller (stessa pipeline locale, `packaging/make-installer.ps1`/`make-uninstaller.ps1`, nessuna logica duplicata) e li allega a una GitHub Release. **Risolve di fatto la riserva sulla riproducibilità** notata in precedenza (`make-installer.ps1` di suo impacchetta il working tree, non un commit preciso): in CI il checkout è sempre pulito (`actions/checkout@v4` su un tag), quindi l'artefatto pubblicato è riproducibile dal tag, anche se lo script in sé resta pensato per "provalo subito" in locale. **`v0.1.0` pubblicata**, allineata all'ultimo commit di `main` (0 commit di distanza), asset `opensagra-installer.exe` + `opensagra-uninstaller.exe` scaricabili dalla pagina Release. C'è anche `.github/workflows/build-wrapper.yml` (solo compilazione del wrapper, artefatto CI, non pubblica) per verificare il wrapper da solo su ogni push che lo tocca.
- [ ] **Il resto resta condizionato**, come deciso: **pubblicità/annuncio** della pubblicazione **solo quando lato codice e lato Windows sono completati e verificati** (il resto di questa Fase 5 più i punti ancora aperti di Fase 3/4) — repo e release sono già online e funzionanti, ma vanno trattati come "non ancora annunciati a un pubblico reale" finché quel lavoro non è chiuso.

### 5c. Guida utente completa

- [x] **Prima stesura (2026-09-14)**: `docs/GUIDA-UTENTE.md` con screenshot reali (non mockup), tabella stampanti aggiornata alla terminologia reale del codice (`USB`/`RETE`/`BRIDGE_NATIVE`/`BLUETOOTH`, non più QZ Tray — rimosso sia da `install.ps1` sia dall'app), e `README.md` corretto in coda allo stesso passaggio (stesso disallineamento QZ Tray).
- [x] **Riscritta e molto ampliata (2026-09-14/15)**: struttura riorganizzata **in ordine di percorso utente reale** su richiesta esplicita (Installazione → Primo avvio/ID cassa → Modalità Indipendente/Client → Gestione Prodotti → Configurazione Scontrino → Configurazione Casse e stampanti → Vendita → Storni → Statistiche), con screenshot granulari per-funzionalità (16 immagini in `docs/img/guida/`, numerate per ordine di lettura — non più le 11 a piena pagina della prima stesura) catturate/riorganizzate con `e2e/_guida_screenshots.manual.js`, incluso un modal "Nuova Cassa" con BRIDGE NATIVO aperto e chiuso senza salvare (nessuna scrittura sul DB). **Correzione di merito emersa scrivendo §3.1**: la frase (presente sia nella bozza precedente della guida sia **testuale nel codice reale di `pages/conf_rete.php`**) "se il PC centrale si spegne le casse client smettono di funzionare finché non torna online" era sbagliata — letto il codice (`pages/billing.php`: `FALLBACK_AFTER_MS_DEFAULT = 15000`, `maybeArmLocalFallback()`, countdown `pos-net-pill-eta` con testo reale `"· DB locale tra Ns"`; `api/set_network_config.php`, `api/enter_local_fallback.php`, `api/push_local_sales.php`, `api/chiudi_cassa.php`) per capire il comportamento vero: switch automatico al DB locale dopo ~15-30s senza perdita vendite, riconnessione al centrale **manuale** (si torna su Configurazione Rete e si riseleziona Client), sync dei debiti pendenti automatico alla chiusura cassa o su richiesta ("Sincronizza ora"). **Corretta anche la copia dell'app stessa** (`pages/conf_rete.php`, non solo la guida) che ripeteva la stessa frase sbagliata.
- **Pubblicata su GitHub il 2026-09-15** (vedi 5b) per essere consultabile con le immagini renderizzate — non un traguardo separato, è la stessa guida di cui sopra.
- [x] **Screenshot reali dei 3 popup Windows + finestra di avanzamento dell'installer (2026-09-16)**: catturati da un'installazione reale (non su VM pulita) e integrati in §1 — SmartScreen ("PC protetto da Windows"), conferma UAC, finestra di avanzamento WPF, e il dialogo nativo di fiducia del certificato. **Osservazione utile per Fase 3g**: il dialogo del certificato **compare comunque** in questo giro nonostante il tentativo `Register-LocalCaTrust`/`frankenphp trust` — non è una controprova definitiva (dipende da quando il browser si apre rispetto al completamento del trust), ma conferma che va trattato in guida come un passaggio atteso, non un'anomalia, cliccando "Sì".
- [x] **Passare certificato caddy a device mobile** fatto ✅ nuovo pagina ad hoc + qr code in pagina conf rete

### 5d. Disinstaller — ✅ bug riprodotti e corretti (2026-09-14)

- [x] **`opensagra-uninstaller.exe` dava errori in un test reale (2026-09-14 mattina)** — isolate e corrette **quattro** cause reali distinte, trovate testando dal vivo su una VM Hyper-V (sia lancio automatizzato sia vero doppio click utente), non solo in teoria:
  1. `Test-Path $logoPath` andava in crash con `$logoPath` vuoto — capita quando lo script gira fuori dal flusso confezionato (commit `f26bff5`).
  2. `winget uninstall --silent` per MariaDB non garantiva un `/quiet` reale sul pacchetto MSI sottostante (si riapriva la finestra dell'uninstaller nativo) — sostituito con `msiexec /x <GUID> /quiet /norestart`, risalendo al product code dal registro Uninstall di Windows, fallback a winget se la voce non c'è (commit `4a2893c`).
  3. Causa più a monte dello stesso crash del punto 1: `$PSScriptRoot` è una **stringa vuota** (non null) nel contesto dell'exe compilato con `ps2exe` — è `Join-Path` stesso a lanciare l'errore con un path vuoto come primo argomento, prima ancora di arrivare al `Test-Path` già protetto. Stesso bug anche in `install.ps1`. Nello stesso giro, scoperto e corretto anche che `mariadbd.exe --install` registra il servizio Windows ma non inizializza mai `data\` (si avviava e restava vuota, il servizio non partiva mai, Event ID 7034) — sostituito con `mariadb-install-db.exe --service=MariaDB`, che fa entrambe le cose in un solo passaggio (commit `9d38ab6`).
  4. Root cause del blocco a tempo indeterminato su "registrazione MariaDB" (CPU a zero, nessun errore): `& exe args 2>&1` da un processo `-noConsole` (l'exe compilato con `ps2exe`) alloca un `conhost.exe` "orfano" per il figlio e l'interazione con la cattura di PowerShell si impianta. Introdotta `Invoke-NativeCaptured` (usa `Start-Process -RedirectStandardOutput/-RedirectStandardError`, pipe/file veri, mai una console) al posto di ogni chiamata diretta a un eseguibile esterno, sia in `install.ps1` sia in `uninstall.ps1` (commit `403eb0f`).
  - **Confermato risolto**: "più cicli install-uninstall-reinstall su VM pulita, anche dopo riavvio" tutti puliti (nota nel commit `d09570e`, un quinto bug trovato nello stesso giro di verifica ma solo lato installer — `data\` non svuotata prima di `mariadb-install-db.exe` in una reinstallazione). Il disinstaller non è più considerato un bug aperto per il rilascio.
  - `docs/GUIDA-UTENTE.md` §12 aggiornata di conseguenza (rimossa la nota "bug noto" per l'utente finale).
...
- [x] **Solo modalità indipendente** (nessun client/server multi-macchina nella demo pubblica — troppa superficie/rischio per una demo esposta). Idea: farla girare in **Docker su TrueNAS** — **da verificare la fattibilità**: il progetto oggi esclude Docker per l'uso reale (serve accesso USB diretto alla stampante), ma una demo pubblica senza stampante fisica collegata al container non ha questo vincolo. Nessuna decisione presa, solo l'idea.
- [x] **Stampa nella demo → niente hardware reale, PDF al posto dello scontrino.** Quando l'utente della demo preme "stampa", un avviso spiega che è una demo pubblica e non è possibile stampare realmente, poi si scarica il PDF dello scontrino invece di instradare verso una stampante vera (riusa il meccanismo già esistente di `print_stat_pdf.php`/dompdf, non il percorso ESC/POS). **Scartata** l'idea di far testare dal vivo il bridge di stampa nativo sulla stampante USB del visitatore — troppo complessa (sicurezza del segreto JWT verso un bridge esterno, isolamento fra visitatori) per il valore che darebbe.
...
Repo locale, branch `main` — **nessun remoto ancora, per scelta: vedi Fase 5b** (pubblicazione su GitHub solo a lato-codice e lato-Windows completati e verificati).

**✅ Chiusa in pratica.** La checklist originale qui sotto elencava messaggi di commit specifici pianificati all'inizio del lavoro, mai spuntati letteralmente — ma nei fatti ogni fase ha chiuso con uno o più commit dedicati (`ca260d0` per la Fase 0, la serie di commit Fase 1/billing, e così via fino ai commit di Fase 4/wrapper visti in dettaglio in 3g/Fase 4). Non serve più tracciarla voce per voce: da qui in avanti il ritmo naturale (un commit per modifica sensata, non per fase) è già la pratica in uso, confermato dal log reale del repo.
...
Script: `e2e/fase_final_verification.manual.js` (smoke test completo) + `e2e/fase_final_remaining.manual.js` (rilancio mirato dei due casi che richiedevano un fix). _(Entrambi rimossi il 2026-09-09 con QZ Tray — questo resoconto resta come registro.)_
...
- [x] **Fase 3** ✅ *(sostanzialmente completata — questa riga era rimasta ferma al 2026-09-07, molto lavoro reale dopo non era stato riportato qui)* — `install.ps1` a zero domande, percorso fisso `C:\opensagra`, testato **per davvero su una VM vergine il 2026-09-12** (5 bug reali trovati e corretti quel giorno, dettaglio in 3g). QZ Tray **rimosso interamente** (2026-09-09, sostituito dal bridge di stampa nativo via Mercure, vedi Fase 4). Packaging finale: **installer a-un-file** `opensagra-installer.exe` + **disinstaller** `opensagra-uninstaller.exe` (entrambi con `ps2exe`, non più la sola bozza `.ps1`). Wrapper Go al posto dei servizi WinSW per FrankenPHP/relay/bridge — completo con finestra di stato, gestione DB (`/db`, AdminNeo), vedi 3g e `docs/PIANO-MODIFICHE-WRAPPER.md`. **Resta**: il dialogo nativo Windows di fiducia del certificato (mitigazione scritta, non riverificata in VM) e un giro end-to-end del wrapper **come client** via installer (oggi verificato solo con config di rete scritta a mano sulla VM).
- [x] **Fase 4** ✅ *(promossa 2026-09-08, ormai completa — questa riga era rimasta ferma al relay "da fare", già fatto da tempo)* — hub Mercure, `publishProductsChanged()`, `EventSource` con fallback al polling: verificato **intra-macchina** e **cross-macchina** (relay, VM). Bridge di stampa nativo: verificato su stampante reale, poi ridisegnato punto-punto (2026-09-12) e verificato su Raspberry Pi + USB. Avviso "server in arresto/tornato" ai client: fatto. Fallback locale una-via + push a chiusura cassa: fatto e testato, incluse le correzioni "vendite mai orfane" e "catalogo che non si sporca" del 2026-09-12. **Runbook end-to-end ripetuto su VM col nuovo gate — ✅ confermato dall'utente (2026-09-14): il meccanismo regge**, restano solo alcuni messaggi di warning da rifinire (non bloccante).
