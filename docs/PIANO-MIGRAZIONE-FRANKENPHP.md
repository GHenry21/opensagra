...
- [x] **Scritta (2026-09-14)**: `docs/GUIDA-UTENTE.md`, con **screenshot reali** catturati da un'installazione dal vivo su questa macchina (Playwright, script una-tantum `e2e/_guida_screenshots.manual.js`, immagini in `docs/images/guida/`) — non mockup/placeholder. Copre: cos'è l'app, requisiti, installazione via `opensagra-installer.exe`, la finestra di stato del wrapper (vista base + "Dettagli avanzati"), uso quotidiano di ogni pagina (Vendite, Storni, Statistiche, Gestione Prodotti), Configurazione Rete (indipendente/client), Configurazione Casse con la tabella scenari stampante **aggiornata alla terminologia reale del codice** (`USB`/`RETE`/`BRIDGE_NATIVE`/`BLUETOOTH` — non più la vecchia tabella basata su QZ Tray, verificato che QZ è stato rimosso sia da `install.ps1` sia dall'app: `conf_casse.php` offre solo `BRIDGE NATIVO`, confermato anche visivamente negli screenshot), lo scenario "servizio al tavolo" (già descritto in 3g, riportato qui in forma per l'utente finale), Configurazione Scontrino, installazione del certificato HTTPS su Android/Windows/Firefox (contenuto di Appendice C e "HTTPS + CA locale su Android" riorganizzato per un lettore non tecnico), il comportamento in caso di caduta rete (§10, sintesi non tecnica del fallback locale — vedi memoria [[fase4_mercure_handoff]]), aggiornamento, disinstallazione **con nota esplicita sul bug noto** (5d sotto, invece di nasconderlo), una tabella di problemi comuni derivata dalle Insidie #1-#8 e dai bug VM di Fase 3g, e dove trovare i log.
- [ ] **Non ancora fatto**: `README.md` è rimasto disallineato (dice ancora che l'installer installa QZ Tray — non più vero, vedi sopra) — andrebbe corretto in un passaggio a parte, non toccato qui per restare nello scope "guida".
