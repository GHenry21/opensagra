...
- [~] **Wrapper/tray-app in Go** — scaffold scritto e poi **compilato con successo 2026-09-09** (Go 1.27 + WinLibs mingw installati sulla macchina dev; `go vet` + `go build -ldflags "-H=windowsgui"` puliti al primo colpo). Anticipato dal punto 5 su richiesta esplicita. Commit `d4d758e` (scaffold), `f9b24da` (announce `--kind=back` all'avvio + "avvia all'accensione"), `7e4c58e`/`332d253` (build + smoke test + rotazione log). **Fatto:** supervisore figli con Job Object `KILL_ON_JOB_CLOSE` + policy di riavvio per exit-code (frankenphp = sempre backoff; relay/bridge exit(0) = idle-recheck lento, exit≠0 = backoff) — verificato con binari fittizi, istanza singola (named mutex), tray con stato live, Esci con conferma + `SHOW PROCESSLIST` + `announce --kind=shutdown`, `--kind=back` all'avvio, "avvia all'accensione", rotazione log dei figli (5 MiB). **Manca:** la finestra di stato/log — **ridisegnata 2026-09-09** da webview incorporata a pagina Vue/HTML servita via HTTP dal wrapper + apertura in browser app-mode (vedi nota sopra: la webview incorporata è stata scartata per il conflitto systray/message-loop, non per un problema di X→tray — quel requisito era comunque più forte del necessario) — non ancora implementata; icona `.ico`; firma `.exe`; test end-to-end con lo stack reale (i tre processi veri, sequenza di uscita, announce, autostart). Dettaglio in `wrapper/README.md`.
  - **Bridge di stampa: esattamente un processo figlio per ID** in `PRINT_BRIDGE_CASSE` (`children.go` ne crea uno per id). Guardia lato script **fatta** (commit `f3d832d`): `bridgeAcquireLock()` in `opensagra-print-bridge.php` prende un `flock(LOCK_EX\|LOCK_NB)` su `<tmp>/opensagra-print-bridge-<cassa>.lock` (ok anche su Windows) ed esce `0` se già preso — copre l'avvio manuale sovrapposto e il riavvio del wrapper prima che il vecchio figlio sia morto. Contesto storico: osservato 2026-09-09, due sessioni di sviluppo col bridge sulla stessa cassa `henry` → due copie identiche di #152.
...
- [x] `install.ps1` + `conf_rete` + wrapper: generare `mercure{}` (con `cookie_name`+`cors_origins`) e `MERCURE_JWT_SECRET` sempre; `conf_rete` sincronizza il segreto via DB al passaggio a client; il **wrapper/tray-app** avvia il relay come processo figlio quando `DB_POS_HOST` è remoto (non più servizio WinSW — vedi revisione 3g). **Fatto 2026-09-09, codice completo, non testato su VM pulita:** `conf_rete` → `7d064bc`; `install.ps1` (`New-CaddyConfig` emette `mercure{}` con `heartbeat 20s` in entrambi i siti, `New-EnvFile` genera `MERCURE_JWT_SECRET` 256-bit hex + righe `_REMOTE`/`PRINT_BRIDGE_CASSE`, `Get-OrNewMercureSecret` lo riusa al rilancio, `Set-MercureSecretInDb` scrive la riga `app_config` dopo il provisioning) → `94c4b2f`; relay come figlio del wrapper → scaffold `d4d758e` (`children.go`).
...
3. **Avviso "il server sta per fermarsi"** ✅ **FATTO 2026-09-08; trigger del wrapper aggiunto 2026-09-09 (scaffold, non compilato).** Topic `cluster/announce`, una-via server→client. Il protocollo Mercure non trasporta il topic al subscriber → ogni payload si auto-descrive con un campo `type` (`'products'` | `'announce'`); `publishProductsChanged()` ora lo aggiunge. `config/mercure.php::publishClusterAnnounce($kind,$eta,$msg)` — `$kind`: `shutdown` | `back`. Trigger: CLI `bin/opensagra-announce.php --kind=shutdown [--eta=30] [--message=…]` (a mano) **oppure il wrapper Go** — `announceShutdown()` prima di terminare FrankenPHP (scaffold `d4d758e`) e `announceBack()` all'avvio quando FrankenPHP risulta su, con qualche tentativo (`f9b24da`); da compilare. Relay: `RELAY_TOPICS` include `cluster/announce`, `on_event` sceglie il topic locale dal `type` (mappa `RELAY_TYPE_TO_TOPIC`), guardia `_relay` invariata. `api/mercure_subscribe_token.php`: scope `subscribe` = `['products','cluster/announce']`, risposta con `topics[]` (retrocompat `topic`). `pages/billing.php`: EventSource con un `topic=` per voce; `onmessage` instrada su `type`; `handleClusterAnnounce()` → `serverAnnounce` → barra fissa in alto (`.server-announce` in `billing.css`, colore `--mc-danger`), chiudibile a mano, si pulisce da sola al primo evento non-announce (server tornato) o dopo 10 min. Cross-macchina: dipende dal segreto Mercure condiviso (manuale finché non c'è il punto 5), come il relay.
