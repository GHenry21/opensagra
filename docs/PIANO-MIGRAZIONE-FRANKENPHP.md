...
- [~] **Wrapper/tray-app in Go** — **scaffold scritto 2026-09-09, non ancora compilato** (nessun toolchain Go sulla macchina dev; serve `cd wrapper && go mod tidy && go build -ldflags "-H=windowsgui" ./...`). Anticipato dal punto 5 su richiesta esplicita. Commit `d4d758e` (scaffold: `main`/`config`/`supervisor`/`children`/`tray`/`cluster`/`platform_windows`/`platform_other`/`util` + README) e `f9b24da` (announce `--kind=back` all'avvio + "avvia all'accensione" via Scheduled Task at-logon). **Fatto nello scaffold:** supervisore figli con Job Object `KILL_ON_JOB_CLOSE` + policy di riavvio per exit-code (frankenphp = sempre backoff; relay/bridge exit(0) = idle-recheck lento, exit≠0 = backoff), istanza singola (named mutex), tray con stato live, Esci con conferma + `SHOW PROCESSLIST` + `announce --kind=shutdown`, `--kind=back` all'avvio, "avvia all'accensione". **Manca:** compilazione/test, finestra stato/log (webview), X→tray (non c'è ancora una finestra), rotazione log, icona `.ico`, firma `.exe`. Dettaglio in `wrapper/README.md`.
