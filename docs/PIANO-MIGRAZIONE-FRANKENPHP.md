...
- [x] Copiare `cert/cert.pem` come certificato di **override** nella cartella di QZ Tray (soppressione popup) — bug reale trovato testando su VM pulita: la cartella dati di QZ Tray non esiste ancora su un'installazione fresca (la crea QZ Tray stesso al primo avvio), il vecchio controllo saltava sempre la copia. Corretto: cartella creata esplicitamente, QZ Tray riavviato per ricaricare l'override se già partito da solo.
- [x] ~~Posizionare la chiave privata in `../../../private/key.pem`~~ **Ridisegnato (2026-09-07)**: quel percorso era calcolato per la vecchia struttura `xampp/htdocs/opensagra` e non aveva senso con il percorso fisso `C:\opensagra` di Fase 3 — il codice che avrebbe dovuto posizionare la chiave, per giunta, non copiava mai nulla da nessuna parte (bug reale, "failed to sign request" da QZ Tray). Invece di limitarsi a correggere il percorso, ogni installazione ora **genera la propria coppia chiave/certificato** (`config/genera_certificati_qz.php`, solo funzioni `openssl_*` di PHP, nessun binario esterno) invece di condividere lo stesso `cert.pem` fisso committato in git tra tutte le installazioni — vedi discussione su licenza QZ Tray (LGPL 2.1) più sotto: nessun impedimento legale, il meccanismo di firma/override è una funzionalità ufficiale documentata da QZ, non un aggiramento. Percorso della chiave centralizzato in `config/qz_key_path.php` (`C:\ProgramData\opensagra\private\key.pem`), usato sia dalla generazione sia da `api/sign-message.php` — prima erano due calcoli indipendenti, causa profonda del disallineamento.
