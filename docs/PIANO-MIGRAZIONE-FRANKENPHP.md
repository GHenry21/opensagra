...
### 5a. Pulizia file e cartelle — primo passo, prima di tutto il resto

- [ ] Ripasso del repo per rimuovere residui di sviluppo prima di qualunque pubblicazione: script `.manual.js`/`.manual.php` superati, file temporanei di test (es. `*.exe~`), cartelle/documenti di debug non più rilevanti, eventuali segreti finiti per sbaglio fuori da `.gitignore`. Non ancora fatto un giro dedicato — finora la pulizia è stata incidentale (es. rimozione `docker/`, script QZ, `fase2d_*`/`fase_final_*` in Fase 3j), serve un passaggio finale mirato al rilascio.
